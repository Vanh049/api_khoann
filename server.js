...
for (let key in obj) normalized[key.toLowerCase()] = obj[key];
...
// ===== xử lý update sinh viên từ trigger =====

if (Array.isArray(data.update)) {

await upsertSinhVien(data.update);

}


// ===== xử lý delete sinh viên từ trigger =====

if (Array.isArray(data.delete)) {

for (const sv of data.delete) {

try {

await pool.query("DELETE FROM sinhvien WHERE masv=$1", [sv.masv]);

writeLog(`Deleted SinhVien via API trigger: ${sv.masv}`);

} catch (e) {

writeLog(`Error deleting SinhVien: ${e.message} - ${JSON.stringify(sv)}`);
...
// ===== xử lý insert/upsert như cũ =====

if (Array.isArray(data.lop)) await upsertLop(data.lop);

if (Array.isArray(data.sinhvien)) await upsertSinhVien(data.sinhvien);

if (Array.isArray(data.dangky)) await upsertDangKy(data.dangky);
